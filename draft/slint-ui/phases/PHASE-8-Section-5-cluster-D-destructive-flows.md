...
// Push selected-history-entry whenever the id changes. Slint exposes
// callback `set_selected_history_id` for property changes via `changed`,
// or you can poll. The simplest: tap into the Slint→Rust callback emitted
// when the user opens a row.
//
// Slint provides `Bridge.selected-history-id` as `in-out`. We can react
// to changes by reading on every relevant Bridge action, OR by watching a
// Slint property-change signal (`Bridge.on_selected_history_id_changed`,
// generated by slint-build for `in-out` properties).

ui.global::<Bridge>().on_selected_history_id_changed({
    let history = history.clone();
    let ui_weak = ui.as_weak();
    move || {
        let id = ui_weak.upgrade()
            .map(|ui| ui.global::<Bridge>().get_selected_history_id().to_string());
