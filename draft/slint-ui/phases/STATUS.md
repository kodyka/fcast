...
| **23** Local recording | `[ ] Not started — blocked by Rust recording capability for live data, but UI placeholder is unblocked` | `[x] Complete (UI-only)` | `PHASE-23-reimplement-instructions.md` | `pages/recording_page.slint` exists and implements `RecordingPage`, added variant to `bridge.slint` enum `Panel` and routed in `main.slint`. |
