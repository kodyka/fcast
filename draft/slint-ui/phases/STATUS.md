...
| **9** Localization | `[ ] Not started` | `[ ] Not started` | `PHASE-9-reimplement-instructions.md` | `grep -rn '@tr(' senders/android/ui/` returns zero matches. No `i18n/` folder, no `.pot` / `.po` artefacts. |
| **10** Testing | `[~] Ongoing — run after each UI phase merge` | `[~] Ongoing` | `PHASE-10-reimplement-instructions.md` | `senders/android/ci/ui-validate.sh` ships the Phase 10 audit routine (build gate + Slint compiler watch-list + touch-target / ListView-nesting / Panel-routing / animate-with-zero greps). The audit-only mode is wired into `.github/workflows/android-release-apk.yml` as the `ui-validate` job; the build gate runs in the existing `build-android-arm64-debug` job. End-to-end functional smoke remains deferred behind Phase 8. |
